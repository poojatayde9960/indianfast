...
const NewOrderRequests = () => {
    ...
    const { data, isLoading } = useGetOrderByShopIdQuery(shopId, {
        skip: !shopId,
    ...
    const handleAcceptOrder = async (orderId) => {
        try {
            ...
            setTimeout(() => navigate("/orders?tab=Ongoing"), 500);
        } catch (error) {
    ...
    const handleRejectOrder = async (orderId) => {
        ...
        try {
            ...
            setTimeout(() => navigate("/orders?tab=Rejected"), 500);
        } catch (error) {
    ...
    const pendingOrders =
        data?.orders?.filter((order) => order.orderStatus === "placeorder") || [];
        ...
        <>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 w-full">
                    ...
                    {!isActive ? (
                        <div className="bg-red-50 border border-red-200 rounded-lg p-6 text-center">
                            <h4 className="text-red-700 font-semibold text-lg mb-2">
                                You are currently Offline
                            </h4>
                            <p className="text-red-600">
                                Please go online to receive and accept new orders.
                            </p>
                        </div>
                        ...
                        pendingOrders.slice(0, 2).map((order, index) => (
                            ...
                            >
                                {/* Prep Time Badge */}
                                {(() => {
                                    const prep = order?.services?.[0]?.products?.[0]
                                        ?.preparationTime
                   ...
                   scrollbar-w-2"
                                        >
                                            {order.services?.[0]?.products?.map((product, pIndex) => (
                                                <div
                                                    key={pIndex}
                                                    className="flex items-center justify-between text-[#000000] font-medium py-0.5 min-h-[28px]"
                                                >
                                                    <div className="flex items-center gap-2">
                                                        <img src={veg} alt="veg" className="w-4 h-4 flex-shrink-0" />
                                                        <span className="text-[15px] sm:text-[16px] truncate max-w-[180px]">
                                                            {product.productName}
                                                            <span className="text-[#00000099] font-normal"> x{product.quantity}</span>
                                                        </span>
                                                    </div>
                                                    <span className="text-[14px] text-[#00000099] whitespace-nowrap">
                                                        ₹{product.total}/-
                                                    </span>
                                                </div>
                                            ))}
                                ...
                                <div className="flex flex-col items-end gap-3 mt-6 sm:mt-0 w-full sm:w-auto">
                                    ...
                                    <div className="absolute bottom-4 left-4 right-4 sm:left-auto sm:right-4 flex flex-wrap sm:flex-nowrap items-center gap-3">
                                        <button
                                        ...
                                        <button
                                            className="h-[40px] sm:h-[48px] w-full sm:w-[150px] rounded-[4px] 
                                                ...
                                                hover:from-[#ff8122] hover:to-[#ffb84d] transition-all shadow-md"
                ...
                <div className="bg-white rounded-[20px] mt-4 lg:-mt-[40%] w-full lg:w-[85%] lg:ml-11 shadow-[0_4px_20px_rgba(0,0,0,0.08)] p-4 sm:p-5">
                    ...
                    <div className="flex justify-between items-center bg-[#D9D9D954] px-3 py-3 text-[13px] sm:text-[14px] font-semibold text-[#000000B2] mb-5">
                        <p>Name</p>
                        <p>Amount</p>
                    </div>
